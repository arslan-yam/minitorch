...
def _tensor_matrix_multiply(
    ...
    * Only write to global memory once per kernel.
    ...
    for block_start in range(0, K, BLOCK_DIM):
        k_a = block_start + pj
        if i < a_shape[1] and k_a < K:
            a_shared[pi, pj] = a_storage[batch * a_batch_stride + i * a_strides[1] + k_a * a_strides[2]]
        else:
            a_shared[pi, pj] = 0.0

        k_b = block_start + pi
        if k_b < K and j < b_shape[2]:
            b_shared[pi, pj] = b_storage[batch * b_batch_stride + k_b * b_strides[1] + j * b_strides[2]]
        else:
            b_shared[pi, pj] = 0.0
